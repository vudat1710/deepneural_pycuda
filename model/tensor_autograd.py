...
class Tensor(object):
    ...
    def __init__(self, data,
        ...
        if tensor_id is None:
            # self.tensor_id = np.random.randint(0, 100000)
            self.tensor_id = np.random.randint(0, 1000000000)
        else:
    ...
    def backward(self, grad=None, grad_origin=None):
        if self.autograd:
            ...
            # only continue backpropagating if there's something to
            # backpropagating into and if all gradients (from children)
            # are accounted for override waiting for children if
            # "backpropagating" was called on this variable directly
